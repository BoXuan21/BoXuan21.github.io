...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["fileToUpload"]) && $_FILES["fileToUpload"]["error"] == 0) {
        ...
        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            ...
            //mysqli_query($con, "INSERT INTO `tbl_products` (`userID`, `productName`, `productCategory`, `productDescription`, `productPrice`, `filename`)
            //  VALUES ($userID, $productName, $productCategory, $productDescription, $productPrice, $filename)");
            // SQL statement to insert data into database
            $sql = "INSERT INTO tbl_products (userID, productName, productCategory, productDescription, productPrice, filename)
                VALUES (?, ?, ?, ?, ?, ?)";
            
            // Prepare and execute the SQL statement
            $stmt = $con->prepare($sql);
            $stmt->bind_param("isssis", $userID, $productName, $productCategory, $productDescription, $productPrice, $filename);
            $stmt->execute();

            header("Location: ../sites/buyProducts.php");
            //echo "Product and file information inserted into database successfully.";
        } else {
